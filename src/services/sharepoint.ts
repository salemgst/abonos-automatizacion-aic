        ...
        if (isNotFound) {
          ...
          try {
            if (previousPath === "") {
              ...
              await msClient()
                .drives.byDriveId(driveId)
                .items.byDriveItemId(parentItemPath)
                .children.post(newFolder);
            }
          } catch (createError) {
...
export async function downloadFromSharePoint(
  ...
  try {
    ...
    const item = await msClient().drives.byDriveId(driveId).items.byDriveItemId(itemPath).get();
 ...
 */
export async function getFileVersions(folderPath: string, fileName: string): Promise<unknown[]> {
  try {
    const { driveId } = config.sharepoint;
    const filePath = `${folderPath}/${fileName}`;
    const itemPath = `root:/${filePath}`;
