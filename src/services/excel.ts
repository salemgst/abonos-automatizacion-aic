 ...
 */
export async function saveWorkbook(workbook: ExcelJS.Workbook, fileName: string): Promise<string> {
...
function insertMovements(
  ...
  movements.forEach((movement) => {
    ...
    const targetRow =
      emptyRowIndex < emptyRows.length ? emptyRows[emptyRowIndex++] : ++currentLastRow;
  ...
  console.log(
    `  ✅ ${movements.length} movimientos insertados (${inEmptyRows} en espacios vacíos, ${atEnd} al final)`
  );
}
